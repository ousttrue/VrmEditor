                          ...
                          const Style& mStyle)
{
  ...
  for (int j = 1; j < 10; j++) {
    recti::Vec2 baseSSpace2 =
      recti::worldToPos(axis * 0.05f * (float)(j * 2) * mCurrent.ScreenFactor,
                        mCurrent.MVP,
                        mCurrent.CameraMouse.Camera.Viewport);
    recti::Vec2 worldDirSSpace2 = recti::worldToPos(
      axis * 0.05f * (float)(j * 2 + 1) * mCurrent.ScreenFactor,
      mCurrent.MVP,
      mCurrent.CameraMouse.Camera.Viewport);
    AddLine(baseSSpace2,
            worldDirSSpace2,
      ...
      */
      ...
      for (unsigned int iCoord = 0; iCoord < 4; iCoord++) {
        cubeFace.faceCoordsScreen[iCoord] = worldToPos(
          faceCoords[iCoord] * 0.5f * invert, res, cameraMouse.Camera.Viewport);
      }
