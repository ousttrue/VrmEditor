         ...
         const gltfjson::tree::NodePtr& arrayNode)
{
  ...
  //
  // bool updated = grapho::imgui::GenericCombo<uint32_t>(label, &selected,
  // span); if (updated) {
  //   if (node) {
                   ...
                   const std::function<bool(size_t i, float* p)>& showGui)
{
  ...
  auto node = parentNode->Get(key);
  if (node) {
    if (std::dynamic_pointer_cast<gltfjson::tree::ArrayNode>(node)) {
      // OK
    } else {
  ...
  if (ImGui::CollapsingHeader(label)) {
    ...
    grapho::imgui::PrintfBuffer buf;
    size_t i = 0;
    for (auto it = array->Value.begin(); it != array->Value.end(); ++i) {
      auto p = (*it)->Ptr<float>();
      if (!p) {
        *it = gltfjson::tree::NewNode(0.0f);
        p = (*it)->Ptr<float>();
      }
      if (showGui(i, p)) {
        updated = true;
