...
VrmGui::ShowExpression()
{
  if (auto ex = m_scene->m_expressions) {
    ...
    const float spacing = 4;
    {
      ...
      s_labels.push_back({
        VSlider(i++, "aa", Enable(lip_aa) ? &lip_aa->weight : nullptr, lipsync),
        "aa",
      });
      auto lip_ih = ex->Get(libvrm::ExpressionPreset::ih);
      s_labels.push_back({
        VSlider(i++, "ih", Enable(lip_ih) ? &lip_ih->weight : nullptr, lipsync),
        "ih",
      });
      auto lip_ou = ex->Get(libvrm::ExpressionPreset::ou);
      s_labels.push_back({
        VSlider(i++, "ou", Enable(lip_ou) ? &lip_ou->weight : nullptr, lipsync),
        "ou",
      });
      auto lip_ee = ex->Get(libvrm::ExpressionPreset::ee);
      s_labels.push_back({
        VSlider(i++, "ee", Enable(lip_ee) ? &lip_ee->weight : nullptr, lipsync),
        "ee",
      });
      auto lip_oh = ex->Get(libvrm::ExpressionPreset::oh);
      s_labels.push_back({
        VSlider(i++, "oh", Enable(lip_oh) ? &lip_oh->weight : nullptr, lipsync),
        "oh",
      });
      // blink
      ...
      s_labels.push_back({
        VSlider(
          i++, "blink", Enable(blink_LR) ? &blink_LR->weight : nullptr, blink),
        "--",
      });
      ...
      s_labels.push_back({
        VSlider(
          i++, "lookUp", Enable(look_up) ? &look_up->weight : nullptr, lookat),
        "󰈈",
      });
