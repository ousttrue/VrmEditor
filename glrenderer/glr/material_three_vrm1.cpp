                       ...
                       const gltfjson::tree::NodePtr& mtoon1)
{
  ...
  *ptr = Material{
    ...
    .FS={
      ...
      .MacroGroups{
        {"TEXTURE", {
          ...
          { "USE_SHADEMULTIPLYTEXTURE", OptVar{[](auto &root, auto, auto &gltf)->std::optional<std::monostate>{
            auto m = gltf.Material();
            if(auto extensions = m.Extensions())
            {
              if(auto ext = extensions->Get(u8"VRMC_materials_mtoon"))
              {
                auto mtoon = gltfjson::vrm1::MToon(ext);
  ...
  auto m = root.Materials[materialId];
